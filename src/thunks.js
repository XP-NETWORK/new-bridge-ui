...
const explorerUrl = (target_nonce, hash) =>
  `${ExplorerPrefix[CHAIN_BY_NONCE[target_nonce]]}/${hash}`;

const callFromInnerSigned = async (
  chain,
  func,
  signer_,
  chain_nonce,
...
export const getWrappedTokensBalances = (chain, address) => async (
  dispatch
) => {
  try {
    const helper = ChainFactory[chain];
 ...
 */
export const sendTokens = (chain, signer_, nonce, to, value) => async (
  dispatch
) => {
  try {
 ...
 * @returns Transaction and the Identifier of this action to track the status
 */
export const returnWrappedTokens = (chain, signer_, nonce, to, value) => async (
  dispatch
) => {
  try {
    let user =
 ...
 * @returns Transaction and the Identifier of this action to track the status
 */
export const sendNFTNative = (chain, sender_, chain_nonce, to, nft) => async (
  dispatch
) => {
  try {
  ...
  return await Promise.all(
    dbList
      .filter(({ data }) => {
        ...
        return (
          attrs[0] === chain_helper.ident &&
          attrs[2].toLowerCase() === owner.toLowerCase()
        );
      })
      .map(async ({ data }) => {
        const attrs = data.split(",");
